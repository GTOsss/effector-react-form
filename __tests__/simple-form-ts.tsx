...
const renderForm = ({ inputRender, form }: { inputRender?: any; form: Form<Values> }) => {
  ...
  const SimpleForm = () => {
    const { handleSubmit, controller, getName } = useForm({ form, meta: { formName: 'simpleForm' } });

    return (
      <form onSubmit={handleSubmit}>
        <Input label="Username" controller={controller({ name: getName('username') })} />
        <Input label="First name" controller={controller({ name: getName('profile', 'firstName') })} />
        <Input label="Last name" controller={controller({ name: getName('profile', 'lastName') })} />
